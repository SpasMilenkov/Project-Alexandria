...
tmux new-window -t "$SESSION_NAME" -n "MediaMetadataWorker" -c "$PWD"
sleep 5.0
tmux send-keys -t "$SESSION_NAME:MediaMetadataWorker" \
  "cd Backend && dotnet run --project Alexandria.Workers.MediaMetadata --launch-profile \"$media_metadata_worker_profile\"" C-m

tmux new-window -t "$SESSION_NAME" -n "LyricsWorker" -c "$PWD"
sleep 5.0
tmux send-keys -t "$SESSION_NAME:LyricsWorker" \
    "cd Backend && dotnet run --project Alexandria.Workers.Lyrics --launch-profile \"$lyrics_worker_profile\"" C-m
