...
public class FileVersionRepository : IFileVersionRepository
{
        ...
        CancellationToken ct = default)
    {
        return await _dbSet.Where(v => v.Id == versionId && v.File.OwnerId == userId)
            .Select(v => new VersionDownloadInfo(v.File.Name, v.File.MimeType, v.VersionNumber, v.ContentHash))
            .FirstOrDefaultAsync(ct);
    }

    public async Task<DownloadMetadata?> GetDownloadMetadataAsync(Guid versionId, Guid userId,
        CancellationToken ct = default)
    {
        return await _dbSet.Where(v => v.Id == versionId && v.File.OwnerId == userId && v.DeletedAt == null).Select(v =>
