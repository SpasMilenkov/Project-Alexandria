    ...
    IUnitOfWork unitOfWork) : IUserSettingsService
{
    ...
    public async Task SetAppearanceAsync(Guid userId, AppearanceSettingsValue value, Guid updatedBy, CancellationToken ct = default)
    {
        Validate(value);
        await UpsertAsync(userId, UserSettingKeys.Appearance, value, updatedBy, ct);
    }

    public async Task SetBehaviorAsync(Guid userId, BehaviorSettingsValue value, Guid updatedBy, CancellationToken ct = default)
    {
        Validate(value);
