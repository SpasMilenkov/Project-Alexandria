...
public class UserRepository(AlexandriaDbContext context, UserManager<ApplicationUser> userManager) : IUserRepository
{
    ...
    public async Task<IEnumerable<ApplicationUser>> AddRangeAsync(IEnumerable<ApplicationUser> entities, CancellationToken ct = default)
    {
    ...
    public async Task<bool> ExistsAsync(Expression<Func<ApplicationUser, bool>> predicate, CancellationToken ct = default)
    {
        return await context.Users.AnyAsync(predicate, ct);
    }

    public async Task<IEnumerable<ApplicationUser>> FindAsync(Expression<Func<ApplicationUser, bool>> predicate, CancellationToken ct = default)
    {
        return await context.Users.Where(predicate).AsNoTracking().ToListAsync(ct);
    }

    public async Task<ApplicationUser?> FirstOrDefaultAsync(Expression<Func<ApplicationUser, bool>> predicate, CancellationToken ct = default)
    {
    ...
    public async Task<PaginatedResult<UserDetailsDto>> GetUsers(UserQueryDto query, CancellationToken ct = default)
    {
        ...
        if (query.CreatedBefore.HasValue)
            dbQuery = dbQuery.Where(x => x.user.CreatedAt < query.CreatedBefore.Value.Date.AddDays(1).ToUniversalTime());
        ...
        if (query.UpdatedBefore.HasValue)
            dbQuery = dbQuery.Where(x => x.user.UpdatedAt < query.UpdatedBefore.Value.Date.AddDays(1).ToUniversalTime());
        ...
        if (query.DeletedBefore.HasValue)
            dbQuery = dbQuery.Where(x => x.user.DeletedAt < query.DeletedBefore.Value.Date.AddDays(1).ToUniversalTime());
        ...
        if (query.LockedOutBefore.HasValue)
            dbQuery = dbQuery.Where(x => x.user.LockoutStartedAt < query.LockedOutBefore.Value.Date.AddDays(1).ToUniversalTime());
